...
Traffic collisions pose a significant public health and safety challenge worldwide, accounting for over 1.35 million deaths annually [@WHO2018]. In major urban centers like Toronto, the complexities of increasing population density, complex transportation networks, and rapid urban development exacerbate the risks associated with road transportation. Understanding the spatial distribution and temporal progression of these collisions is critical for identifying risk factors and implementing interventions that enhance road safety.

Toronto’s policy environment provides a unique case for examining changes in collision dynamics over time. In 2017, the city launched the Vision Zero Road Safety Plan, aiming to eliminate traffic fatalities and serious injuries. Although various measures—ranging from infrastructure upgrades to public awareness campaigns—have been introduced, Toronto still experiences high rates of collisions, injuries, and fatalities [@CityOfToronto2017]. Previous research has explored influences on collision rates, including driver behavior, weather conditions, and roadway design [@Ma2019; @Wazana2020]. However, these analyses often do not delve into the granular neighborhood level or systematically assess how policy interventions affect localized patterns over time, leaving a critical gap in our understanding of Vision Zero’s impact at a fine spatial scale.
...
1. **Traffic Collision Data**: Detailed records of reported traffic collisions in Toronto from January 2014 to December 2021, obtained from the **City of Toronto’s Open Data Portal** (City of Toronto 2022a). This dataset is produced by the Toronto Police Service for annual statistical reporting and public transparency, making it a comprehensive and official source for understanding collision occurrences. While other datasets (e.g., national databases, insurance claim records, or community-reported incidents) could potentially offer additional insights, these alternatives often lack detailed spatial resolution, consistent reporting standards, or the necessary temporal coverage that the Toronto dataset provides.
...
2. **Toronto Neighborhood Boundaries**: Geospatial data defining the boundaries of Toronto's 140 neighborhoods, sourced from the **City of Toronto's Open Data Portal** (City of Toronto 2022b). These boundaries are used administratively and are stable over time, allowing for longitudinal neighborhood-level analysis.
...
- **INJURY_COLLISIONS**: Indicator if the collision involved injuries, providing a quick severity check ("YES"/"NO") (**factor**).
- **AUTOMOBILE**, **MOTORCYCLE**, **CYCLIST**, **PEDESTRIAN**: Indicators for the types of road users involved ("YES"/"NO") (**factors**). “YES” indicates that at least one party of that type was involved.
...
- **GEOMETRY**: Spatial polygon that defines the neighborhood’s boundaries. These boundaries facilitate stable geographic comparisons over time (**sf object**).
...
1. **Merging Datasets**:
   - Used a spatial join to assign collisions to neighborhoods. This step leverages the GEOMETRY of neighborhoods and the LAT/LONG of collisions.
...
# Convert OCC_DATE and filter missing coords
...
The table below provides a high-level snapshot of the dataset’s scale, temporal coverage, and spatial coverage. It illustrates the number of records, average fatalities per collision, the earliest and latest years of data, and how many neighborhoods are represented.
...
#| cache: TRUE
#| fig-pos: "H"
summary_stats <- collisions_clean %>%
  summarize(
    tot_recs = n(),
    avg_fat = mean(FATALITIES, na.rm = TRUE),
    min_y = min(OCC_YEAR),
    max_y = max(OCC_YEAR),
    unique_neigh = n_distinct(NEIGHBOURHOOD_NAME)
  )

knitr::kable(summary_stats, caption = "Summary Statistics of Key Variables", booktabs = TRUE) %>%
  kableExtra::kable_styling(latex_options = c("hold_position"))
```

## **Descriptive Analysis and Visualizations**

Below, we present a selection of visualizations to better understand distributions and patterns in the data. We focus on temporal patterns, spatial distributions, severity breakdowns, and involvement of vulnerable road users. While we refrain from interpretive explanations here, these visuals lay the foundation for insights discussed later.
...
#| fig-cap: Annual number of traffic collisions reported in Toronto from 2014 to 2021
#| echo: false
#| cache: TRUE
#| fig-pos: "H"
#| fig.width: 6
...
#| fig-cap: Map displaying total collisions per neighborhoods in Toronto
#| echo: false
#| cache: TRUE
#| fig-pos: "H"
#| fig.width: 6
...
#| fig-cap: Bar chart of collision severities in Toronto
#| echo: false
#| cache: TRUE
#| fig-pos: "H"
#| fig.width: 5.6
...
#| fig-cap: Yearly trends in collision severity(2014-2021)
#| echo: false
#| cache: TRUE
#| fig-pos: "H"
#| fig.width: 6
...
#| fig-cap: Annual Collisions Involving Vulnerable Road Users Over Time
#| echo: false
#| cache: TRUE
#| fig-pos: "H"
#| fig.width: 6
...
- **Underreporting and Data Bias**: Minor collisions or those without injuries may be underreported. This could lead to underestimation of collision frequencies, especially for property damage-only incidents [@Elvik1999; @Alsop2001].
...
The model was implemented using the glm.nb() function from the MASS package in R [@MASS].
...
neighbourhood_yearly_collisions$NEIGHBOURHOOD_NAME <- droplevels(neighbourhood_yearly_collisions$NEIGHBOURHOOD_NAME)
...
The Negative Binomial model estimates are summarized below. This model includes year effects, a post-policy indicator (PostVisionZero), and severity-related rates. Neighborhood-specific effects are also included but not displayed here for brevity.

**Table NB: Negative Binomial Regression Estimates**
...
The coefficient plot below shows point estimates and confidence intervals for the main predictors of interest, providing a visual representation of their direction and uncertainty.
...
coef_data <- broom::tidy(nb_model) %>%
  filter(term %in% c("OCC_YEAR","PostVisionZero","InjuryRate","FatalityRate"))

ggplot(coef_data, aes(x = term, y = estimate, ymin = estimate - 1.96*std.error, ymax = estimate + 1.96*std.error)) +
  geom_pointrange() +
  coord_flip() +
  labs(x = "Predictor",
...
To assess the model fit, we conducted a residual analysis by plotting the Pearson residuals against the fitted values, plot belo.

```{r}
#| label: fig:residual-fitted-plot
#| echo: false
...
We performed 5-fold cross-validation to evaluate the predictive performance of the model. The results are summarized in **Table 5**.
...
knitr::kable(cv_results, caption = "Cross-Validation Performance Metrics")
```
...
**Table 7: Cross-Validation Performance Metrics**
